...
### M23.5 (2026-07-07) — 核心逻辑: 兑换船票消耗勾选的物品

用户手机自查修正 M23.4 错误方向. 核心逻辑:

- **行李** = `_selectedGiftIds` (未兑换的礼物)
- **兑换船票** = 把勾选的物品从行李里**去掉** (消耗)
- **归家之心检查** = 看**剩余行李**里有没有 id=5

修复:

1. 回滚 M23.4 的 line 1751 `hasHomeHeart = selectedIds[5] || _ticketExchanged` 改动
   (M23.4 错把'兑换船票 = 有归家之心', 完全跟用户原话相反)
2. 核心修复 line 1268-1273 兑换按钮:
   - 之前: 只设 `_ticketExchanged=true`, 不动 `_selectedGiftIds`
   - 现在: 兑换时 `_selectedGiftIds = []` (消耗全部勾选的) + `_selectedCount = 0` + `_selectedPrice = 0`

行为表:

| 场景 | 兑换时勾了 id=5? | 剩余行李有 id=5? | voyage 行为 |
|---|---|---|---|
| 收集 id=5, 没勾 | 没勾 | 有 | 到 Bandar (有心) |
| 收集 id=5, **勾它**兑换 | 勾了 | 没 | 中点返航 (无心) |
| 没收集 id=5, 用其他物品换 | (没 id=5 可勾) | 没 | 中点返航 (无心) |

后续关卡: 没勾选的物品保留在 `_selectedGiftIds`, 关卡间沿用.
