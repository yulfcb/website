// 哈萨克斯坦·草原游牧贸易 —— 关 1 游戏引擎 (M4: 货币系统 + 水壶可视化)
//
// 重做原因：原 Iran 关是纯 DOM 卡片游戏，太简单。仿关 0 (Qatar) 的 Phaser 模式：
// 玩家控制人物在沙漠地图上行走，
// - 走访 🏦 交易中心把行李物品换成坚戈 (₸)
// - 用坚戈在 6 家哈萨克商贩 (地毯/马奶酒/茶/陶/骆驼/水壶) 买商品
// - 走访 2 个绿洲给水壶灌水
// - 集齐 **2 个水壶 + 2 壶都满 10L** 启程去土耳其 (Altai 阿尔泰边境)
// - 携带行李箱 (quantity-based 同类型可多件) 跨级
//
// M4 范围：
//   - 货币系统: 玩家有 `this.coins` 余额, 中心用行李物品换坚戈, 商贩收坚戈卖货
...
//     绿洲自动把最空的水壶灌满 (除非都满)
//   - 出口条件: 2 壶 + 2 壶都满水 + 走到左上方阿尔泰 → camera fade → 跳下一关
...
(function () {
  ...
  if (!window.KAZAKHSTAN_MODE) {
    console.warn('[kazakhstan] window.KAZAKHSTAN_MODE not set, abort');
    return;
  }
  var L = window.KAZAKHSTAN_LEVEL;
  var Q = window.QATAR_LEVEL;  // 商品本体定义从 QATAR_LEVEL.gifts 来 (8 件)
  if (!L) {
    console.error('[kazakhstan] window.KAZAKHSTAN_LEVEL missing, abort');
    return;
  }
  if (!Q || !Q.gifts) {
    console.error('[kazakhstan] window.QATAR_LEVEL.gifts missing, abort');
    return;
  }
  var LEVEL_ID = 1;
  ...
  var BootScene = new Phaser.Class({
    ...
    create: function () {
      var self = this;
      // 哈萨克夜空深蓝背景 — Boot 阶段过渡
      this.cameras.main.setBackgroundColor('#2E5D1F');
      this.add.text(640, 360, '哈萨克斯坦·草原游牧贸易\n加载中…', {
  ...
  var PlayScene = new Phaser.Class({
    ...
    create: function () {
      ...
      L.merchants.forEach(function (m) {
        var halo = self.add.graphics();
        halo.fillStyle(0x1B5E8A, 0.4);  // 哈萨克蓝
      ...
      if (!window.KAZAKHSTAN_AVATARS[avatarId]) avatarId = 'malay';
      ...
      // 任务提示
      this.add.text(640, 80, '🎯 去交易中心换坚戈 → 购买 2 个水壶 → 灌满水 → 阿尔泰过境', {
      ...
      this.time.delayedCall(800, function () {
        self.showToast('💡 先去右边的交易中心 🏦 把行李换成坚戈', 2800);
      });
    ...
    _loadLuggage: function () {
      ...
      console.log('[kazakhstan] luggage loaded:', JSON.stringify(merged));
      return merged;
    },
    ...
    tryMove: function (key) {
      ...
      var steps = this.camelMode ? 2 : 1;  // 骆驼每次走 2 格
      var dx = 0, dy = 0;
      if (key === 'up') dy = -step * steps;
      else if (key === 'down') dy = step * steps;
      else if (key === 'left')  { dx = -step * steps; this.player.facing = -1; }
      else if (key === 'right') { dx = step * steps;  this.player.facing = 1; }
        ...
        if (Math.sqrt(dx * dx + dy * dy) < 80) {
          if (!o._lastTouch || Date.now() - o._lastTouch > 2000) {
            ...
            if (filled !== null) {
              ...
              window.playKazakhstanSfx('pickup', 0.4);
            } else {
    ...
    _renderExchangeModal: function () {
      ...
      this.modalContainer.add(this.add.text(0, -200, '把行李物品换成坚戈 ₸', {
        fontSize: '13px', color: '#C9B89A', fontStyle: 'italic',
      }).setOrigin(0.5));
      ...
      // 提示行
      this.modalContainer.add(this.add.text(0, 170, '👆 点击物品 → 卖出一件 → 获得坚戈', {
        fontSize: '13px', color: '#F4ECD8', fontStyle: 'italic',
      }).setOrigin(0.5));
    ...
    // M4: 商店买入 — 玩家付 price 坚戈, 获得 1 份商贩卖品
    //   - 卖品用 -1000 - m.id 作为内部 id 进 luggage (跟 M3 兼容)
    //   - 水壶商 (id=5) 额外给一个空水壶
    doBuy: function (m) {
      if (this.coins < m.price) {
        this.showToast('坚戈不够！去交易中心 🏦', 1500);
        window.playKazakhstanSfx('click', 0.3);
        return;
      }
      // 1) 扣钱
    ...
    // 合并 QATAR_LEVEL.gifts + KAZAKHSTAN 自定义商品 (地毯/马奶酒/...) 用于显示
    ...
    departIran: function () {
      ...
      window.playKazakhstanSfx('exchange', 0.4);
      ...
      var msg = this.add.text(640, 360, '🚩 阿尔泰 → 土耳其 🇹🇷\n启程…', {
    ...
    showDepartToast: function () {
      this.showToast('启程条件已满足！前往阿尔泰…', 1500);
    },
